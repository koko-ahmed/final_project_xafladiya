  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    if (searchBtn) {
      searchBtn.addEventListener('click', function() {
        ...
        // Get all event cards
        const eventCards = document.querySelectorAll('.venue-card');
        ...
        eventCards.forEach(card => {
          ...
          // Check location
          if (locationValue && showCard) {
            const cardLocation = card.querySelector('.venue-location').textContent.toLowerCase();
            if (!cardLocation.includes(locationValue.toLowerCase())) {
              showCard = false;
            }
          }
          ...
          // Check date
          if (date && showCard) {
            const cardDate = card.querySelector('.venue-date').textContent;
            const cardDateObj = new Date(cardDate);
            const searchDateObj = new Date(date);
            if (cardDateObj.toDateString() !== searchDateObj.toDateString()) {
              showCard = false;
            }
          }
        ...
        const visibleCards = document.querySelectorAll('.venue-card[style="display: block"]');
        ...
        if (visibleCards.length === 0) {
          if (!noResultsMsg) {
            ...
            document.querySelector('#dynamic-events .row').appendChild(message);
          }
